...
def register_compliance_routes(bp, admin_required_decorator):
    ...
    def compliance_latest_combined():
        ...
        try:
            ...
            raw_results = compliance_service.get_scan_results(scan_id) if scan_id else []
            results = [dict(r) if not isinstance(r, dict) else r for r in raw_results]
    ...
    @bp.route("/compliance/report/<scan_id>")
    @login_required
    @require_tier("pro")
    ...
    @bp.route("/api/compliance/report/<scan_id>/pdf")
    @login_required
    @require_tier("pro")
