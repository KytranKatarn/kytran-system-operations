...
def get_scan_report_data(scan_id):
    ...
    try:
        ...
        # -- build rule description map from rule packs --------------------
        rule_desc_map = {}
        rule_pack_rows = conn.execute("SELECT pack_id, rules FROM compliance_rule_packs").fetchall()
        for rp in rule_pack_rows:
            rpd = dict(rp)
            rules_data = rpd.get("rules", "[]")
            if isinstance(rules_data, str):
                rules_data = json.loads(rules_data)
            for rule in rules_data:
                rule_desc_map[rule.get("rule_id", "")] = rule.get("title", "")

        # Add titles to findings
        for f in findings:
            f["title"] = rule_desc_map.get(f.get("rule_id", ""), "")
        ...
        for ev in ev_rows:
            d = dict(ev)
            # Add rule title from rule_desc_map using the rule_id column
            rule_id = d.get("rule_id", "")
            d["type"] = d.get("evidence_type", "check")
            d["description"] = rule_desc_map.get(rule_id, "")
            # Parse content JSON for additional details
            if not d["description"]:
                try:
                    ev_data = json.loads(d.get("content", "") or "{}")
                    d["description"] = ev_data.get("actual", "")[:200]
                except (json.JSONDecodeError, TypeError):
                    d["description"] = d.get("content", "")[:200]
            key = d.get("soc2_mapping") or "Unmapped"
            evidence.setdefault(key, []).append(d)

        # -- alias scan fields for template compatibility -------------------
        scan["id"] = scan.get("scan_id", "")
        scan["overall_score"] = scan.get("score", 0)
        scan["scanned_at"] = scan.get("completed_at", "")
        ...
        try:
            hm_path = os.path.join(
                os.environ.get("ARCHIE_BASE_DIR", "/data"),
                "host_monitor_data.json"
            )
            if os.path.exists(hm_path):
                with open(hm_path) as hf:
                    hm_data = json.load(hf)
                sys_info = hm_data.get("system", {})
                if isinstance(sys_info, dict):
                    hostname = sys_info.get("hostname", hostname)
                    os_info = f"{sys_info.get('distribution', 'Linux')} {sys_info.get('kernel', '')}"
                else:
                    hostname = str(sys_info) if sys_info else hostname
        except Exception:
            pass
        server = {
            "hostname": hostname,
        ...
        try:
            ...
            start = _dt.fromisoformat(str(scan.get("started_at", "")))
            end = _dt.fromisoformat(str(scan.get("completed_at", "")))
            scan["duration"] = round((end - start).total_seconds(), 1)
        except Exception:
            scan["duration"] = None

        # -- remediation history for this scan ------------------------------
        remediations = []
        try:
            rem_rows = conn.execute(
                "SELECT * FROM compliance_remediations WHERE scan_id = ? ORDER BY executed_at",
                (scan_id,),
            ).fetchall()
            remediations = [dict(r) for r in rem_rows]
        except Exception:
