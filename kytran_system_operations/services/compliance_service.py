...
Rule packs are synced from ARCHIE (source of truth) when connected.
Falls back to local packs when standalone.
"""
...
# ---------------------------------------------------------------------------
# Rule pack sync from ARCHIE hub
# ---------------------------------------------------------------------------
_last_pack_sync = None


def sync_rule_packs_from_hub():
    """Fetch rule packs from ARCHIE and upsert into local DB.

    Called before each scan when hub is connected. Skips if synced
    within the last 5 minutes to avoid hammering the API.
    """
    global _last_pack_sync
    import time
    if _last_pack_sync and (time.time() - _last_pack_sync) < 300:
        return  # Synced recently

    try:
        from .hub_client import is_hub_configured
        if not is_hub_configured():
            return

        import requests
        from flask import current_app
        hub_url = current_app.config.get("KSO_ARCHIE_HUB_URL", "")
        if not hub_url:
            hub_url = os.environ.get("KSO_ARCHIE_HUB_URL", "")
        if not hub_url:
            return
        ...
        for pack in packs:
            ...
            db.execute(
                """INSERT INTO compliance_rule_packs (pack_id, name, rules)
                   VALUES (?, ?, ?)
                   ON CONFLICT(pack_id) DO UPDATE SET name = ?, rules = ?""",
                (pack_id, name, rules_json, name, rules_json),
            )
        db.commit()
        db.close()
        _last_pack_sync = time.time()
        logging.getLogger(__name__).info("Synced %d rule packs from ARCHIE hub", len(packs))
    except Exception as exc:
        logging.getLogger(__name__).warning("Rule pack sync error: %s", exc)
...
def run_scan(pack_ids=None, triggered_by="manual"):
    ...
    """
    # Sync rule packs from ARCHIE if connected
    try:
        sync_rule_packs_from_hub()
    except Exception:
        pass  # Continue with local packs
...
def _check_command_output(check):
    ...
    try:
        ...
        # Host-level paths must run on the host, not inside the container
        _HOST_PATHS = ("/etc/ufw/", "/etc/sysctl", "/proc/sys/", "/etc/ssh/",
                       "/etc/audit/", "/etc/pam.d/", "/etc/security/", "/etc/modprobe",
                       "/etc/default/ufw", "ip neigh", "ufw status", "ufw show")
        _needs_host = any(hp in command for hp in _HOST_PATHS)

        if cmd_base in _SAFE_LOCAL and not _needs_host:
            try:
...
def apply_fix(scan_id, rule_id, user_id):
    ...
    try:
        if fix_type == "file_line":
            ...
            command_executed = f"file_line: {path} → {line}"
            params = {"path": path, "line": line}
            if restart:
                params["restart_service"] = restart
                command_executed += f" && restart {restart}"
            _run_host_cmd("compliance_fix_file_line", params)

        elif fix_type == "command":
            command_executed = fix_def.get("command", "")
            if not command_executed:
                return {"success": False, "error": "No command in fix definition"}
            _run_host_cmd("compliance_fix_command", {"command": command_executed}, timeout=60)

        elif fix_type == "sysctl":
            param = fix_def.get("param", "")
            value = fix_def.get("value", "")
            command_executed = f"sysctl -w {param}={value}"
            _run_host_cmd("compliance_fix_sysctl", {"param": param, "value": value})
            ...
            _run_host_cmd("compliance_fix_service", {"service": service, "action": action})
...
def generate_ckl(scan_id):
    ...
    with _db_cursor(commit=False) as (conn, cur):
        ...
        _raw = cur.fetchone()
        scan = dict(_raw) if _raw else None
    ...
    pack_names = {}
    with _db_cursor(commit=False) as (conn, cur):
        cur.execute("SELECT pack_id, name FROM compliance_rule_packs")
        for _row in cur.fetchall():
            row = dict(_row)
            pack_names[row["pack_id"]] = {"name": row["name"], "version": "V1R1"}
